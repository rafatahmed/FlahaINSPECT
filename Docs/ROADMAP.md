...
**Current release in progress:** **R1** (branch `feat/pr-03-schema`). R0 tag: `r0-design-freeze`. R1-01 and R1-02 merged.
...
- [x] Tile *policy* locked (vendor name remains G-01)
...
| R1-03 | PR-03 | Drizzle + raw SQL schema, triggers, seeds | R1-02 | `in-progress` | Migration applies twice (expand-safe); seed uses `SEED_PASSWORD`; unique active-report index |
...
- G-01 tile vendor (blocks offline maps only)
